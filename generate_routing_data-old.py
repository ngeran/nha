...
def _build_inet0_table(
    count: int, used_prefixes: set, shared_prefixes: list | None = None
) -> list:
    """
...
def generate_device_data(
    ...
    print(
        ...
        f"({active} active" + (f", {shared} shared prefixes" if shared else "") + ")"
    )
...
def _prompt_int(
    prompt: str, default: int, min_val: int = 1, max_val: int = 10_000
) -> int:
    while True:
...
def generate_routing_data(
    ...
    if total_routes is None:
        total_routes = _prompt_int(
            "How many inet.0 routes per device?",
            default=150,
            min_val=10,
            max_val=10_000,
        )
    ...
    if overlap:
        if num_devices is None:
            num_devices = _prompt_int(
                "How many device files to generate?",
                default=3,
                min_val=2,
                max_val=len(DEVICE_NAMES),
            )
